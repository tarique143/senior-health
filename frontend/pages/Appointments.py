# /frontend/pages/Appointments.py (Final Version)
...
for app in st.session_state.get('appointments', []):
    calendar_events.append({
        ...
        "start": app['appointment_datetime'], "end": app['appointment_datetime'],
        "extendedProps": {"id": app['id']}
    })

calendar_options = {
    "headerToolbar": {"left": "prev,next today", "center": "title", "right": "dayGridMonth,timeGridWeek,timeGridDay,listMonth"},
    "initialView": "dayGridMonth", "slotMinTime": "06:00:00", "slotMaxTime": "22:00:00",
...
clicked_event = calendar(
    ...
    custom_css="""
        .fc-event-past { opacity: 0.7; } .fc-event-time { font-style: italic; }
...
if st.session_state.get('edit_mode_app'): expander_title = "✏️ Edit Selected Appointment"
...
with st.expander(expander_title, expanded=st.session_state.get('edit_mode_app', False)):
    ...
    if edit_app_id and 'appointments' in st.session_state:
        ...
        if app_to_edit:
            ...
            default_values = {
                "doctor_name": app_to_edit.get('doctor_name', ''), "purpose": app_to_edit.get('purpose', ''),
                "location": app_to_edit.get('location', ''), "date": dt_obj.date(), "time": dt_obj.time()
            }
        ...
        with b_col1:
            if st.form_submit_button("💾 Save Appointment", type="primary", use_container_width=True):
                if not doctor_name: st.warning("Doctor's Name is required.")
                    ...
                    app_data = {
                        "doctor_name": doctor_name, "purpose": purpose, "location": location,
                    ...
                    st.session_state.edit_mode_app = False; st.session_state.pop('edit_app_id', None)
                    fetch_appointments(); st.rerun()
        with b_col2:
            if st.session_state.get('edit_mode_app') and st.form_submit_button("🗑️ Delete", use_container_width=True):
                ...
                fetch_appointments(); st.rerun()
        with b_col3:
            if st.session_state.get('edit_mode_app') and st.form_submit_button("✖️ Cancel", use_container_width=True):
                st.session_state.edit_mode_app = False; st.session_state.pop('edit_app_id', None)
                st.rerun()
